...
struct PLAY_STATE {
  struct PLAY_CHANNEL channels[4];

  unsigned char should_loop_mod;
  unsigned char mod_ended;
  
  unsigned char jump_enabled;
  unsigned char jump_to_song_pos;
  unsigned char jump_to_row;
...
static uint32_t play_frequency;
static uint32_t sample_advance_factor;
static struct PLAY_STATE state;
static const struct MOD_DATA *mod;

static int start_row(void);

// returns fixed point 20.12
static uint32_t get_sample_advance_for_period(unsigned int period)
{
  return sample_advance_factor / period;
}

static uint32_t get_samples_per_tick(unsigned int bpm)
{
  return play_frequency * 5 / bpm / 2;
}
...
{
  mod = mod_data;
  play_frequency = out_frequency;
  sample_advance_factor = (((((uint32_t)MOD_REF_CLOCK)/2)<<10) / out_frequency)<<2;

  reset_channels();
  
  state.should_loop_mod = loop;
  state.mod_ended = 0;
  ...
  state.cur_pattern = mod->song_positions[state.cur_song_pos];
  state.cur_row = 0;
...
static void process_channel_effect(int chan_num)
{
  ...
  case 0x6: break; // vibrato into volume slide (processed in ticl)
...
static int start_row(void)
{
  if (state.jump_enabled) {    // process jump to new song position
    ...
    if (state.jump_to_song_pos > state.cur_song_pos || state.should_loop_mod) {
  ...
  if (state.cur_row >= 64) {   // scroll to next song position
    state.cur_row = 0;
    if (++state.cur_song_pos >= mod->num_song_positions) {
      if (! state.should_loop_mod) {
        state.mod_ended = 1;
        return 1;
      }
      state.cur_song_pos = 0;
  ...
  const struct MOD_CELL *cell = &mod->pattern[mod->num_channels * (64*state.cur_pattern + state.cur_row)];
  for (int c = 0; c < mod->num_channels; c++) {
    struct PLAY_CHANNEL *ch = &state.channels[c];
    if (cell->sample > 0) {
      ch->sample = &mod->samples[cell->sample-1];
      ch->sample_pos = 0;
